...
boolean isLotFree; // boolean do define if the parking lot is free or not
boolean initialized = false; // initialized flag
...
void loop() {
 ...
 if(!initialized || (Sonar >= 20 && !isLotFree) || (Sonar < 20 && isLotFree)){
  initialized = true;
  isLotFree = Sonar >= 20;
  Serial.println((isLotFree ? msg + String(test_lot_number) + String("\", \"status\":\"") + status_free + String("\", \"token\":\"") + security_token + String("\"}") : msg + String(test_lot_number) + String("\", \"status\":\"") + status_occupied + String("\", \"token\":\"") + security_token + String("\"}"))); 
 }


//  if (Sonar >= 20 &&  !isLotFree) {
